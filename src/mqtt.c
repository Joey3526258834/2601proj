...
int mqtt_init()
{
    ...
    if(MQTTCLIENT_SUCCESS !=rc )
    {
        printf("Failed to connect MQTTClient_setCallbacks, return code %d\n", rc);
        exit(EXIT_FAILURE);
    }

    if ((rc = MQTTClient_connect(client, &conn_opts)) != MQTTCLIENT_SUCCESS)
    {
        printf("Failed to connect MQTTClient_connect, return code %d\n", rc);
        exit(EXIT_FAILURE);
    }
...
int mqtt_send(char * key, float value)
{
